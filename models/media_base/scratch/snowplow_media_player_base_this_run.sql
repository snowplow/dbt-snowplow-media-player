...
with prep as (
  ...
  from {{ ref('snowplow_media_player_base_events_this_run') }} as i
...
, retention_rate as (
  ...
  inner join {{ ref("snowplow_media_player_base_events_this_run") }} as i
...
, duration_fix as (
  ...
  from {{ ref('snowplow_media_player_base_events_this_run') }} as f
...
select
  ...
  case
    when d.play_time_sec > {{ var("snowplow__valid_play_sec") }} then true else
      false
  end as is_valid_play,
  case
    when
      d.play_time_sec / nullif(f.duration, 0)
  ...
  -- to correct incorrect result due to duplicate session_id (one removed)
  end, 0) as {{ type_float() }}) as retention_rate,
  d.seeks,
  case
    when d.percent_progress_reached = '' then null else
      d.percent_progress_reached
  end as percent_progress_reached
